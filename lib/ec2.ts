...
export class SimpleEC2 extends cdk.Construct {
  constructor(scope: cdk.Construct, id: string, ec2props: SimpleEC2Props) {
    ...
    const role = new iam.Role(
      this,
      `${ec2props.ec2InstanceName}-role`,
      { assumedBy: new iam.ServicePrincipal('ec2.amazonaws.com') }
    );

    const ecrPermission = new iam.PolicyStatement();
    ecrPermission.addResources(ec2props.ecrRepo.repositoryArn);
    ecrPermission.addActions(`ecr:GetAuthorizationToken`,`ecr:GetDownloadUrlForLayer`,`ecr:BatchGetImage`);
